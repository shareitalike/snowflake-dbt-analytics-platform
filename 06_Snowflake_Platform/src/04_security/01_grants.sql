 ...
 * 
 * EXPLANATION: Enforces Least Privilege access rights by explicitly granting USAGE, SELECT, and DML permissions on databases, schemas, and tables to specific roles.
 * DESIGN DECISIONS: Grants DBT_SERVICE full DML capabilities in Silver and Gold, but strictly limits Analysts (BI_DEVELOPER, BUSINESS_ANALYST) to SELECT access only on views in the Gold layer.
 * WHY: Restricting analysts to views ensures data governance policies (like Row Access or Dynamic Data Masking) cannot be bypassed by querying the underlying tables directly. It also prevents accidental modification of production data.
 * ============================================================================== */
