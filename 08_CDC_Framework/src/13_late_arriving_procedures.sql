/* ==============================================================================
 * FILE: 13_late_arriving_procedures.sql
 * PHASE: 08 - CDC Framework
 * 
 * EXPLANATION: Implements the Inferred Member (Ghost Dimension) logic to handle Late Arriving Data, ensuring fact tables can load even if parent dimension records haven't arrived yet.
 * DESIGN DECISIONS: Uses a LEFT JOIN anti-join pattern instead of NOT IN for massive performance gains at scale. The procedure runs immediately before the Fact table merges to proactively create placeholder "Ghost" records.
 * WHY: In distributed microservice architectures (like Shopify/Stripe), event arrival order is not guaranteed. Creating ghost dimensions preserves referential integrity (foreign keys) in the Gold layer without dropping or pausing fact data.
 * ============================================================================== */
