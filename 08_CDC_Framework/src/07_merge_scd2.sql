/* ==============================================================================
 * FILE: 07_merge_scd2.sql
 * PHASE: 08 - CDC Framework (Module 4)
 * 
 * EXPLANATION: Implements Slowly Changing Dimension (SCD) Type 2 logic for the Customer dimension using a two-step MERGE process within an explicit transaction.
 * DESIGN DECISIONS: Wraps the expiration (UPDATE) and insertion (INSERT) MERGE statements in a BEGIN/COMMIT block. Uses MD5 hashing for surrogate keys and record checksums, and handles errors with an explicit ROLLBACK and pipeline log update.
 * WHY: The explicit transaction ensures the dimension is never left in a half-expired state if a warehouse timeout occurs. The surrogate key generation uses effective dates, guaranteeing uniqueness across versions, and deterministic keys for ghost records.
 * ============================================================================== */
