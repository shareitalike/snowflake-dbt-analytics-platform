 ...
 * 
 * EXPLANATION: Orchestrates the CDC transformations using a Snowflake Task DAG, moving data from Bronze to Silver (Curated) through a defined dependency chain.
 * DESIGN DECISIONS: Uses a serverless root task triggered every 15 minutes that only logs a checkpoint. Child tasks rely on AFTER clauses for dependencies and WHEN SYSTEM$STREAM_HAS_DATA to trigger warehouse compute only if data exists.
 * WHY: Serverless root tasks avoid continuous warehouse spin-up costs (FinOps). The condition WHEN SYSTEM$STREAM_HAS_DATA ensures compute is strictly pay-as-you-go. The dependency chain ensures referential integrity (e.g., Ghost records inferred before Orders are merged).
 * ============================================================================== */
