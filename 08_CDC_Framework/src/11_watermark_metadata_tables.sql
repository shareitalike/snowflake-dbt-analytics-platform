/* ==============================================================================
 * FILE: 11_watermark_metadata_tables.sql
 * PHASE: 08 - CDC Framework
 * 
 * EXPLANATION: Defines the metadata control schema for managing pipeline executions, specifically the pipeline registry, global high watermarks, and individual batch control checkpoints.
 * DESIGN DECISIONS: Normalizes operational metadata into three tables (Registry, Watermark, Batch Control) linked by Pipeline_ID. Batch Control records the precise start/end bounds and row counts for each run.
 * WHY: Centralizing pipeline metadata is essential for an idempotent, restartable CDC framework. Tracking High/Low watermarks guarantees exactly-once processing semantic boundaries, while the Batch table provides an irrefutable audit trail of exactly what was processed when.
 * ============================================================================== */
