/* ==============================================================================
 * FILE: 12_watermark_procedures.sql
 * PHASE: 08 - CDC Framework
 * 
 * EXPLANATION: Provides the core reusable stored procedures (Start Checkpoint, Commit Checkpoint, Rollback Checkpoint) to orchestrate stateful batch boundaries for CDC pipelines.
 * DESIGN DECISIONS: SP_CREATE_CHECKPOINT fetches the last successful High_Watermark to use as the new Low_Watermark. SP_UPDATE_CHECKPOINT atomically commits the batch and uses a MERGE statement to upsert the new global High_Watermark.
 * WHY: Wrapping watermark management in standard stored procedures enforces a uniform state-management pattern across all data pipelines. It prevents race conditions and ensures that failed pipeline runs do not advance the watermark, allowing seamless retries.
 * ============================================================================== */
