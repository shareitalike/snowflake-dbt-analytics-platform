 ...
 * 
 * EXPLANATION: Defines CDC Streams (Append-Only and Standard) on raw Bronze tables to track data changes across different source systems (Shopify, POS, Stripe, Salesforce, Oracle, etc.).
 * DESIGN DECISIONS: Utilizes APPEND_ONLY = TRUE for sources ingested via Snowpipe (which only inserts), while using Standard Streams for sources synced via tools like Fivetran/Matillion that perform DML (UPDATE/DELETE).
 * WHY: Using APPEND_ONLY streams for append-only data significantly reduces Snowflake metadata tracking overhead, improving performance. Standard streams are strictly reserved for sources where historical updates or deletes occur directly in the raw layer.
 * ============================================================================== */
