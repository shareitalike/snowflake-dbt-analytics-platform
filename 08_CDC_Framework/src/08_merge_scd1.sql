/* ==============================================================================
 * FILE: 08_merge_scd1.sql
 * PHASE: 08 - CDC Framework (Module 4)
 * 
 * EXPLANATION: Implements Slowly Changing Dimension (SCD) Type 1 logic (overwrite) for reference data like Currency Conversions.
 * DESIGN DECISIONS: Uses a single MERGE statement with METADATA$ACTION and METADATA$ISUPDATE to handle inserts, updates, and soft deletes from a Standard Stream. Uses QUALIFY to deduplicate multiple changes in a single micro-batch.
 * WHY: SCD1 is appropriate for reference data where historical tracking is unnecessary. Relying on stream metadata enables accurate handling of source deletions (soft deletes) and updates without complex timestamp comparisons.
 * ============================================================================== */
