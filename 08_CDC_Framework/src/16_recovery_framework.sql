/* ==============================================================================
 * FILE: 16_recovery_framework.sql
 * PHASE: 08 - CDC Framework
 * 
 * EXPLANATION: Provides disaster recovery utilities for state management, specifically allowing manual rollback of Watermarks and the recovery of STALE Snowflake streams.
 * DESIGN DECISIONS: SP_RECOVER_STALE_STREAM dynamically reconstructs a stale stream using AT (TIMESTAMP => ...) pointing exactly to the last known good Watermark. Validates user input against the TB_PIPELINE_REGISTER to prevent SQL injection.
 * WHY: If a stream is suspended for >14 days (or >90 with extended retention), it goes STALE and cannot be resumed. This framework allows engineers to perfectly rebuild the stream exactly where it left off, preventing data loss or duplication.
 * ============================================================================== */
