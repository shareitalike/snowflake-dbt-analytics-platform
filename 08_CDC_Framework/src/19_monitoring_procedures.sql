/* ==============================================================================
 * FILE: 19_monitoring_procedures.sql
 * PHASE: 08 - CDC Framework
 * 
 * EXPLANATION: Defines stored procedures to evaluate pipeline health (SLA breaches, Ghost proliferation) and aggregate historical pipeline execution metrics.
 * DESIGN DECISIONS: Uses idempotent MERGE statements for daily metric rollups rather than standard INSERTs. Actively sweeps the VW_PIPELINE_FRESHNESS_SLA view and pushes critical breaches into the TB_ALERT_QUEUE while preventing duplicate alert spam.
 * WHY: Idempotency in metric rollups ensures that if a task fails halfway and is rerun, the dashboard data is not duplicated. Deduping alerts at the database layer prevents "alert fatigue" in downstream systems like PagerDuty or Slack.
 * ============================================================================== */
