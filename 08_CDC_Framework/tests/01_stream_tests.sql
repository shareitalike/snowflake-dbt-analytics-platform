/* ==============================================================================
 * FILE: 01_stream_tests.sql
 * PHASE: 08 - CDC Framework
 * 
 * EXPLANATION: Unit tests validating the behavior of Snowflake Streams, explicitly testing both Append-Only streams (for JSON payloads) and Standard streams (for transactional data).
 * DESIGN DECISIONS: Tests consume the stream inside a BEGIN...COMMIT block to safely simulate a task execution without permanently losing the stream data if the test fails midway.
 * WHY: Validating stream behavior post-deployment ensures the streams were configured correctly. Append-Only streams silently ignore updates to the base table, which can cause severe data loss if a Standard stream was actually required.
 * ============================================================================== */
