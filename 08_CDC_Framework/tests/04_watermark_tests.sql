/* ==============================================================================
 * FILE: 04_watermark_tests.sql
 * PHASE: 08 - CDC Framework
 * 
 * EXPLANATION: Unit tests for the Watermark and Checkpoint framework, simulating initial loads, incremental commits, failed runs, and duplicate batch prevention.
 * DESIGN DECISIONS: Simulates a pipeline failure by calling SP_ROLLBACK_CHECKPOINT and then proves that a subsequent SP_CREATE_CHECKPOINT fetches the exact same Low_Watermark rather than advancing.
 * WHY: Correct state management prevents data loss or duplication. Proving that the watermark does *not* advance on a failed run is critical to ensuring the pipeline can self-heal without manual intervention.
 * ============================================================================== */
