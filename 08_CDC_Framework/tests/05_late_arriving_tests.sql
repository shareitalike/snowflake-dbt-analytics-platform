/* ==============================================================================
 * FILE: 05_late_arriving_tests.sql
 * PHASE: 08 - CDC Framework
 * 
 * EXPLANATION: Unit tests verifying the "Ghost Dimension" logic handles late arriving dimension data (e.g. an order arrives before the customer profile).
 * DESIGN DECISIONS: Injects an order with a non-existent customer_id, runs the inference procedure to create the Ghost, merges the order, and finally injects the actual customer data to prove the Ghost is properly expired and overwritten (SCD2).
 * WHY: Late arriving data is a common edge case in distributed systems. Proving the Ghost logic works ensures that Fact data is never dropped or quarantined indefinitely simply because a downstream API was slow.
 * ============================================================================== */
