"""
==============================================================================
FILE: marketing_pipeline_dag.py
PHASE: 11 - Airflow Orchestration

EXPLANATION: An Airflow DAG designed to pull marketing campaign data from external REST APIs (e.g., Marketo) into Snowflake.
DESIGN DECISIONS: Utilizes the SimpleHttpOperator for lightweight API calls instead of building a massive custom Python operator. Implements high retry counts with jitter (retry_delay=1) to handle strict API rate limits gracefully.
WHY: Third-party APIs are notoriously flaky and prone to rate-limiting (HTTP 429). Configuring aggressive retries at the Airflow level prevents the entire DAG from failing due to a momentary network blip.
