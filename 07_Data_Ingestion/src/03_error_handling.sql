/* ==============================================================================
 * FILE: 03_error_handling.sql
 * PHASE: 07 - Data Ingestion
 * 
 * EXPLANATION: Implements a stored procedure to identify and replay failed files from the Snowflake copy history, logging persistent failures to a Dead Letter Queue (DLQ).
 * DESIGN DECISIONS: Uses a JavaScript stored procedure to query INFORMATION_SCHEMA.COPY_HISTORY, dynamically construct a COPY INTO ... FORCE=TRUE statement, and catch/log errors to a Quarantine schema.
 * WHY: Automating the replay process reduces manual operational overhead. Pushing persistently failed records to a DLQ ensures no data is silently dropped, enabling data engineers to investigate and fix issues without halting the main pipeline.
 * ============================================================================== */
