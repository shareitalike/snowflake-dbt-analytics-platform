...
EXPLANATION: Functions as a Tier 1 Pre-Flight Validator to guarantee incoming DataFrames strictly match the expected target schema before any compute-heavy logic runs.
DESIGN DECISIONS: Evaluates the DataFrame schema locally using Snowpark (df.schema.fields) without executing a query on the Snowflake warehouse. Supports a strict mode to either fail on unexpected columns or allow additive schema evolution.
WHY: Catching data type drift (e.g., an upstream system changing a numeric column to a string) before executing a massive Snowpark transformation saves significant warehouse compute credits and prevents corrupting the Silver layer.
==============================================================================
"""
import logging
