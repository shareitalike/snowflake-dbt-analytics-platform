...
EXPLANATION: An independent observer class designed to periodically query Snowflake metadata (Information Schema) to verify Service Level Agreement (SLA) compliance for data freshness.
DESIGN DECISIONS: Decoupled entirely from the actual data pipelines. It checks the LAST_ALTERED timestamp of critical Gold tables against defined maximum delay thresholds.
WHY: If an Airflow scheduler dies, the pipelines won't run, meaning they won't report failures. An independent SLA monitor running via a native Snowflake Task guarantees that if data stops arriving for any reason, the business is alerted.
==============================================================================
"""
import logging
