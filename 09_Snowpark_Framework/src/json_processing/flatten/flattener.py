...
EXPLANATION: Provides a Snowpark utility to flatten nested JSON arrays dynamically using Snowflake's native LATERAL FLATTEN function.
DESIGN DECISIONS: Uses the table_function("flatten") within the Snowpark DataFrame API. Accepts an outer parameter to dictate whether to preserve parent rows that contain empty arrays (similar to a LEFT JOIN).
WHY: Manually writing SQL to flatten highly nested JSON structures is brittle and error-prone. Encapsulating this in an object-oriented Python class allows for dynamic path resolution and reusable transformations across multiple data domains.
==============================================================================
"""
import logging
