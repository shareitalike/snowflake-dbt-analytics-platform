...
EXPLANATION: The master execution wrapper for the Snowpark framework. It dynamically injects Audit, Logging, and Metrics tracking around core business logic.
DESIGN DECISIONS: Implements the Context Manager pattern (with self.audit_manager...) to guarantee that pipeline status (START, SUCCESS, FAILED) is logged to the control tables even if the underlying business logic encounters a catastrophic exception.
WHY: Developers frequently forget to add try/except blocks or logging to their specific scripts. Using an orchestrator forces all data pipelines to conform to enterprise observability standards without relying on developer discipline.
==============================================================================
"""
import logging
